...
function FloatingNote({ note, index, colors }: { note: DeskStickyNote; index: number; colors: any }) {
  ...
  return <Animated.View style={[styles.note, note.isUnavailable && styles.unavailableNote, { backgroundColor: note.isUnavailable ? colors.surfaceSoft : colors.amberSoft, transform: [{ translateY }, { rotate }] }]}><Text numberOfLines={3} style={[styles.noteText, note.isUnavailable && { color: colors.muted, fontStyle: "italic" }, { color: note.isUnavailable ? colors.muted : colors.text }]}>{note.text}</Text><Text numberOfLines={1} style={[styles.author, { color: note.isUnavailable ? colors.muted : colors.violet }]}>{note.isUnavailable ? "CONTENT UNAVAILABLE" : `${note.author.anonymousUsername} · ${note._count.applauds} applause`}</Text></Animated.View>;
}

const styles = StyleSheet.create({
  shell: { borderWidth: 1, borderRadius: 19, padding: 17, marginTop: 14 }, header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 12 }, eyebrow: { fontSize: 10, fontWeight: "900", letterSpacing: 1.1 }, title: { fontSize: 15, fontWeight: "900", marginTop: 4 }, open: { fontWeight: "900", fontSize: 12 }, loader: { height: 92, justifyContent: "center" }, ticker: { gap: 10, paddingTop: 14, paddingRight: 4 }, note: { width: 164, minHeight: 92, borderRadius: 10, padding: 11, justifyContent: "space-between" }, unavailableNote: { borderWidth: 1, borderColor: "rgba(113, 97, 84, .18)" }, noteText: { fontSize: 12, lineHeight: 17, fontWeight: "700" }, author: { fontSize: 9, fontWeight: "900", marginTop: 8 }, empty: { marginTop: 14, borderRadius: 10, padding: 13 }, emptyText: { fontSize: 12, lineHeight: 18 },
});
