...
export default function ChatScreen() {
  ...
  const [reportedPromptId, setReportedPromptId] = useState<string | null>(null);
  ...
  async function reportPromptAnswer() { if (!token || !connection?.prompt || reportBusy || reportedPromptId === connection.prompt.id) return; try { setReportBusy(true); await reportContent(token, "PROMPT_ANSWER", connection.prompt.id, "Inappropriate friendship question answer"); setReportedPromptId(connection.prompt.id); Alert.alert("Report received", "A moderator will review this shared answer."); } catch (error: any) { Alert.alert("Couldn’t report answer", error?.response?.data?.message ?? "Please try again."); } finally { setReportBusy(false); } }
    ...
    >
      ...
      <FlatList
        ...
        ListHeaderComponent={isDirect && connection ? <View style={[styles.connectionCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          {connection.prompt?.status === "COMPLETED" && <View style={[styles.promptCard, { borderColor: colors.border }]}><Text style={[styles.promptLabel, { color: colors.teal }]}>SHARED ANSWERS · {connection.prompt.targetLabel.toUpperCase()}</Text><Text style={[styles.promptQuestion, { color: colors.text }]}>{connection.prompt.question}</Text><Text style={[styles.answerLabel, { color: colors.muted }]}>Your answer</Text><Text style={[styles.answerText, { color: colors.text }]}>{connection.prompt.myAnswerUnavailable ? "Removed by a moderator." : connection.prompt.myAnswer}</Text><Text style={[styles.answerLabel, { color: colors.muted }]}>Their answer</Text><Text style={[styles.answerText, { color: colors.text }]}>{connection.prompt.memberAnswerUnavailable ? "Removed by a moderator." : connection.prompt.memberAnswer}</Text>{!connection.prompt.memberAnswerUnavailable && <TouchableOpacity disabled={reportBusy || reportedPromptId === connection.prompt.id} accessibilityRole="button" onPress={reportPromptAnswer} style={styles.reportAnswer}><Text style={[styles.reportAnswerText, { color: colors.danger }]}>{reportedPromptId === connection.prompt.id ? "Answer reported" : reportBusy ? "Reporting…" : "Report their answer"}</Text></TouchableOpacity>}</View>}
