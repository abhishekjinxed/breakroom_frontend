...
export default function InboxScreen() {
  ...
  const load = useCallback(async () => { if (!token) return; try { setLoadError(null); setItems(await getInbox(token)); } catch (error: any) { setLoadError(error?.response?.data?.message || "Unable to load Inbox. Check your connection and try again."); } finally { setLoading(false); } }, [token]);
  useEffect(() => { load(); }, [load]);
