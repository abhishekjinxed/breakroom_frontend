...
export default function InboxScreen() {
  ...
  const load = useCallback(async () => { if (!token) return; try { setLoadError(null); setItems(await getInbox(token)); } catch (error: any) { setLoadError(error?.response?.data?.message || t("inboxLoadFailed")); } finally { setLoading(false); } }, [token, t]);
  useEffect(() => { load(); }, [load]);
