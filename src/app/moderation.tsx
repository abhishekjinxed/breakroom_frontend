...
import { ActivityIndicator, Alert, Image, Modal, RefreshControl, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View } from "react-native";
...
function ReportCard({ report, working, colors, onResolve, onDisable }: { report: ModerationReport; working: boolean; colors: any; onResolve: (report: ModerationReport, status: "REVIEWED" | "DISMISSED") => void; onDisable: (report: ModerationReport) => void }) {
  ...
  return <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
    ...
    {!!report.target.imageUrl && <Image accessibilityLabel="Reported profile photo" source={{ uri: report.target.imageUrl }} resizeMode="contain" style={styles.reportImage} />}
