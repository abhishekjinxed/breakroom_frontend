...
function PulseVideo({ source }: { source: string }) {
  const browserSafeSource = source.includes("/video/upload/")
    ? source.replace("/video/upload/", "/video/upload/f_mp4,vc_h264,ac_aac/")
    : source;
  const player = useVideoPlayer(browserSafeSource, (video) => { video.loop = true; });
  return <VideoView player={player} style={styles.media} nativeControls playsInline contentFit="contain" />;
}
...
export default function OfficePulseScreen() {
  ...
  const [media, setMedia] = useState<{ mediaUrl: string; mediaType: "IMAGE" | "VIDEO" } | null>(null);
  ...
  async function chooseMedia() {
    try { const selection = await pickAndUploadMedia(); if (selection) setMedia(selection); }
  ...
  async function publish() {
    ...
    try {
      setPublishing(true);
      const pulse = await createPulse(token, { text: text.trim(), ...media });
      setPulses((current) => [pulse, ...current]); setText(""); setMedia(null);
  ...
  return <SafeAreaView style={styles.safeArea}>
    ...
    <FlatList data={pulses} keyExtractor={(item) => item.id} refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor={Brand.colors.teal} />}
      contentContainerStyle={styles.list} ListHeaderComponent={<View style={styles.composer}>
        <Text style={styles.composerTitle}>Share a Work Pulse</Text><TextInput value={text} onChangeText={setText} placeholder="What are you working on or taking a break from?" placeholderTextColor={Brand.colors.muted} style={styles.composerInput} multiline maxLength={500} />
        {media && <View style={styles.attachment}><Text style={styles.attachmentText}>{media.mediaType === "VIDEO" ? "Video attached" : "Photo attached"}</Text><TouchableOpacity onPress={() => setMedia(null)}><Text style={styles.remove}>Remove</Text></TouchableOpacity></View>}
        <View style={styles.composerActions}><TouchableOpacity onPress={chooseMedia}><Text style={styles.mediaAction}>＋ Add photo or video</Text></TouchableOpacity><TouchableOpacity onPress={publish} disabled={publishing || (!text.trim() && !media)} style={styles.shareButton}><Text style={styles.shareButtonText}>{publishing ? "Sharing…" : "Share pulse"}</Text></TouchableOpacity></View>
      </View>}
      ...
      renderItem={({ item }) => <View style={styles.card}>
        <View style={styles.authorRow}><View><Text style={styles.author}>{item.author.anonymousUsername}</Text><Text style={styles.time}>WORK PULSE</Text></View><TouchableOpacity onPress={() => safetyActions(item)}><Text style={styles.more}>•••</Text></TouchableOpacity></View>
        {!!item.text && <Text style={styles.pulseText}>{item.text}</Text>}
        {item.mediaUrl && (item.mediaType === "VIDEO" ? <PulseVideo source={item.mediaUrl} /> : <Image source={{ uri: item.mediaUrl }} style={styles.media} resizeMode="cover" />)}
        ...
        {item.notes.map((note) => <View key={note.id} style={styles.note}><Text style={styles.noteAuthor}>{note.author.anonymousUsername}</Text><Text style={styles.noteText}>{note.text}</Text></View>)}
