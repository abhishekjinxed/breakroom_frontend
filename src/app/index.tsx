...
function FocusedHome({ username, profilePhotoUrl, token, colors, onLogout }: { username: string; profilePhotoUrl?: string | null; token: string | null; colors: any; onLogout: () => Promise<void> }) {
  ...
  return <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.canvas }]}><ScrollView contentContainerStyle={styles.focusedContent}>
    ...
    <TouchableOpacity onPress={() => router.push("/tic-tac-toe" as any)} style={[styles.coffeeBreakCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.coffeeBreakIcon, { backgroundColor: colors.tealSoft }]}><Text style={[styles.coffeeBreakIconText, { color: colors.teal }]}>⊞</Text></View><View style={styles.coffeeBreakCopy}><Text style={[styles.coffeeBreakEyebrow, { color: colors.teal }]}>OPEN GAME TABLE</Text><Text style={[styles.coffeeBreakTitle, { color: colors.text }]}>Tic-Tac-Toe</Text><Text style={[styles.coffeeBreakText, { color: colors.muted }]}>A quick anonymous match when you have a few minutes to spare.</Text></View><Text style={[styles.coffeeBreakArrow, { color: colors.teal }]}>→</Text></TouchableOpacity>
    <DeskNotesTicker />
