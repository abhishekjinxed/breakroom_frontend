...
function FocusedHome({ username, profilePhotoUrl, token, colors, onLogout }: { username: string; profilePhotoUrl?: string | null; token: string | null; colors: any; onLogout: () => Promise<void> }) {
  ...
  return <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.canvas }]}><ScrollView contentContainerStyle={styles.focusedContent}>
    ...
    <TouchableOpacity onPress={() => router.push("/coffee-break" as any)} style={[styles.coffeeBreakCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.coffeeBreakIcon, { backgroundColor: colors.amberSoft }]}><Text style={[styles.coffeeBreakIconText, { color: colors.amber }]}>☕</Text></View><View style={styles.coffeeBreakCopy}><Text style={[styles.coffeeBreakEyebrow, { color: colors.teal }]}>COFFEE BREAK ROOMS</Text><Text style={[styles.coffeeBreakTitle, { color: colors.text }]}>Take five minutes with a small group</Text><Text style={[styles.coffeeBreakText, { color: colors.muted }]}>Private, temporary chats for 3–5 people. Continue with a red Charter Plane only if you choose.</Text></View><Text style={[styles.coffeeBreakArrow, { color: colors.teal }]}>→</Text></TouchableOpacity>
    <TouchableOpacity onPress={() => router.push("/games" as any)} style={[styles.coffeeBreakCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.coffeeBreakIcon, { backgroundColor: colors.tealSoft }]}><Text style={[styles.coffeeBreakIconText, { color: colors.teal }]}>⊞</Text></View><View style={styles.coffeeBreakCopy}><Text style={[styles.coffeeBreakEyebrow, { color: colors.teal }]}>OPEN GAME TABLE</Text><Text style={[styles.coffeeBreakTitle, { color: colors.text }]}>Games</Text><Text style={[styles.coffeeBreakText, { color: colors.muted }]}>A quick anonymous match or a solo Tetris break when you have a few minutes.</Text></View><Text style={[styles.coffeeBreakArrow, { color: colors.teal }]}>→</Text></TouchableOpacity>
    <DeskNotesTicker />
