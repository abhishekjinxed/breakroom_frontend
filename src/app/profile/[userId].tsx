...
import { ActivityIndicator, Alert, Animated, Image, Linking, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View } from "react-native";
...
export default function PublicProfileScreen() {
  ...
  function sendCharter() { if (!token || !userId || sending) return; Alert.alert("Send Charter Plane?", `A red Charter Plane will be sent directly to ${profile?.anonymousUsername ?? "this member"}'s desk. It costs 100 Paisa (10× a regular plane) and remains there for 24 hours.`, [{ text: "Cancel", style: "cancel" }, { text: "Send for 100 Paisa", style: "destructive", onPress: () => send() }]); }
  async function send() { if (!token || !userId || sending) return; try { setSending(true); const result = await sendCharterPaperPlane(token, userId); Alert.alert("Charter Plane sent", `A red Charter Plane is now on ${profile?.anonymousUsername ?? "their"} desk for 24 hours. ${result.wallet.balance} Paisa remaining.`); } catch (error: any) { Alert.alert("Charter Plane", error?.response?.data?.message ?? "Please try again."); } finally { setSending(false); } }
...
const styles = StyleSheet.create({
  ...
  charterButton: { backgroundColor: "#B8443F" }, charterHelp: { color: "#FFF0E8", fontSize: 10, fontWeight: "700", marginTop: 4 },
});
